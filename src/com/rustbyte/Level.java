...
public class Level {	
	...
	public Level(Bitmap bitmap, int tw, int th, Game g) {
		...
		for(int i=0; i < (width * height);i++) {
			Tile t = new Tile(i % width, i / width, tileWidth, tileHeight,
					Tile.getTileTypeFromID(bitmap.pixels[i] & 0xFFFFFF), this);
			t.tileID = i;
			map[i] = t;
		}
	}
	...
	public void draw(Bitmap dest) {
		for(int i=0; i < (width * height); i++) {
			Tile t = (Tile)map[i];
			if( t.typeID == Tile.TILE_NONE.typeID) continue;
